  ...
  const fetchMetersData = async () => {
    ...
    try {
      const response = await axios.get(
        `${baseURL}/singlemeterView/dailybillingData`,
        {
          params: {
      ...
      console.error("API Error (daily billing Report):", error);
      setMetersData([]);
    } finally {
      setLoading(false);
    }
      ...
      >
        ...
        {loading ? (
          ...
          <DataGrid
            ...
            getRowId={(row, index) =>
              row.billingDatetime
                ? `${row.billingDatetime}-${index ?? Math.random()}`
            ...
            componentsProps={{
              toolbar: { csvOptions: { fileName: "DailyBillingReport" } },
            }}
            density="compact"
            sx={{
...
export default DailyBillingReport;
