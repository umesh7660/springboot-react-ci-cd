...
const RealTimeTampers = () => {
  ...
  const fetchMetersData = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`${baseURL}/data/tamperSummary`, {
        params: {
      ...
      console.error("API Error (Real Time Report):", error);
      setMetersData([]);
    } finally {
      setLoading(false);
    }
  ...
  >
     <Typography variant="h3" fontWeight="bold" className='mb-3'>
             Real Time Tampers Report Details
      </Typography>
    ...
    >
      ...
      {loading ? (
        ...
        <DataGrid
          ...
          componentsProps={{
            toolbar: { csvOptions: { fileName: "RealTimeTampersReport" } },
          }}
          density="compact" 
          sx={{
...
export default RealTimeTampers;
